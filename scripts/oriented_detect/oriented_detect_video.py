#!/usr/bin/env python3
"""YOLO oriented object detection on live video or a video file.

Runs YOLO OBB on frames from a webcam, RTSP stream, or video file and displays
annotated results in an OpenCV window.

Example:
    python scripts/oriented_detect/oriented_detect_video.py --source 0
    python scripts/oriented_detect/oriented_detect_video.py --source path/to/video.mp4
"""
...
DEFAULT_SOURCE = "0"
WINDOW_NAME = "YOLO Oriented Detection"


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the oriented detection video demo."""
    parser = argparse.ArgumentParser(description="YOLO oriented detection video demo")
    parser.add_argument(
        "--model",
        default=DEFAULT_MODEL,
        help="YOLO oriented detection model checkpoint",
    )
    parser.add_argument(
        "--source",
...
def main() -> None:
    """Run YOLO oriented detection on a video stream."""
    args = parse_args()
    ...
    while True:
        ...
        annotated = annotate_obb(results[0])
